...
void enqueue(int data, int priority)
{
    ...
    if (isEmpty()) // empty list; front rear is the newnode,
    {
        front = newNode;
        rear = newNode;
    }
    else if (priority < front->priority) // highest priority, add before front, front= newnode,
    {
        newNode->link = front;
        front = newNode;
    }
    ...
    {
        temp = front;
        while (temp->link != NULL && temp->link->priority <= priority) // move the temp to the position,newnode will add after temp
            temp = temp->link;                                         // only do this for while
        if (temp->link == NULL)
        {
            temp->link = newNode;
            rear = newNode;
