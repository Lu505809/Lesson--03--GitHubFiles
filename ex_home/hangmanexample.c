...
int i_rnd(int i)
{
	...
	return rand() % i; // create rand function!
}

char *decrypt(char *code)// function with pointer return value,decrypt is the guessed word
{
	int hash = ((strlen(code) - 3) / 3) + 2;//hash is the number of code??
	char *decrypt = malloc(hash); // assign a specified amount of memory for an array to be created
	char *toFree = decrypt;
	char *word = code;
	for (int ch = *code; ch != '\0'; ch = *(++code))//ch is a pointer to code adresse, and loop for every lettre of code
	{
		if ((code - word + 2) % 3 == 1)//??
		{
	...
	*decrypt = '\0';
	return toFree; // create 3 pointer
}

void printBody(int mistakes, char *body)//function to creat the body when there is a mistake
{
	printf("\tMistakes :%d\n", mistakes);
	switch (mistakes)
	{
...
void printWord(char *guess, int len)//guess is the guess word
{
	printf("\t");
	for (int i = 0; i < len; ++i)
	{
...
int main()
{
	...
	char values[WORDS][WORDLEN] = {"N~mqOlJ^tZletXodeYgs", "gCnDIfFQe^CdP^^B{hZpeLA^hv", "7urtrtwQv{dt`>^}FaR]i]XUug^GI",
								   ...
								   "OHkttvxdp|[nnW]Drgaomdq"};// libray of the words,
	...
	memset(body, ' ', CHANCE + 1);// copies the '' to the body for length 'chance+'
